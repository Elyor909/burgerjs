...
for(let key in product) {
    if(product[key].amount == 0) {
        cartListItem.innerHTML = '<h3 class="wrapper__navbar-checklist-text">Пусто</h3>'
    }
}
...
window.addEventListener('click', (event) => {
    if(!event.target.closest('.wrapper__navbar-basket') && !event.target.closest('.wrapper__navbar-btn')) {
